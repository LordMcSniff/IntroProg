...
Canvas color_corners(Canvas c)
{
    ...
    c = canvas_set_black(c, 0, h - 1);
    c = canvas_set_black(c, w - 1, h - 1);
    c = canvas_set_black(c, w - 1, 0);
...
Canvas descending_diagonal(Canvas c)
{
    for (size_t y = 0; y <= canvas_height(c); y++)
    {
        ...
        {
            c = canvas_set_black(c, x, canvas_height(c) - y);
        }
    }
    return c;
}
...
Canvas draw_rectangle(Canvas c, int px, int py, int width, int height)
{
    for (size_t x = 0; x < width; x++)
    {
        for (size_t y = 0; y < height; y++)
        {
            ...
            // limit positiv
            if ((px + x) >= canvas_width(c))
                continue;
            if ((py - y) >= canvas_height(c))
                continue;

            c = canvas_set_black(c, px + x, py - y);
        }
    }
