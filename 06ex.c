...
Canvas swatch_red_green(Canvas c) {
    ...
    for (size_t y = 0; y < h; y++)
    {
        ...
        {
            c = canvas_set_r(c, x, y, x / (w - 1.0f));
            c = canvas_set_g(c, x, y, y / (h - 1.0f));
        }
    }
...
Canvas canvas_set_channel(Canvas c, int x, int y, ColorChannel channel, float v) {
    ...
    default:
        printf("unknown channel: %i", channel);
        break;
    }
    return c;
}
...
Canvas swatch_rgb(Canvas c, ColorChannel channel_x, ColorChannel channel_y) {
    ...
    for (size_t y = 0; y < h; y++)
    {
        for (size_t x = 0; x < w; x++)
        {
            canvas_set_channel(c, x, y, channel_x, x / (w - 1.0f));
            canvas_set_channel(c, x, y, channel_y, y / (h - 1.0f));
        }
    }
...
RGB rgb_to_gray(RGB color) {
    float grey = color.r * 0.2989f + color.g * 0.587f + color.b * 0.1140f;
    RGB out = {grey, grey, grey};
    return out;
}
...
Canvas canvas_to_gray(Canvas c) {
    for (size_t y = 0; y < canvas_height(c); y++)
    {
        for (size_t x = 0; x < canvas_width(c); x++)
        {
            ...
            RGB grey = rgb_to_gray(color);
            canvas_set_rgb_struct(c, x, y, grey);
        }
    }
