...
{
    // da das Struct RGB keine pointer enthält wird es vom compiler continues / gleich in beiden Fällen x und y im speicher festgelegt,
    // das heist r, g und b werden immer den gleichen offset zu dem struct pointer haben, egal bei welcher instance des structs.
    // return &(x->b) == &(y->b);
    return x == y;
}
...
int32_t frumpleflabblefizzledipp(int32_t x)
{
    ...
    frumple(flabble(&x, &fizz), &v27, &out);
...
{
    PileOfPancakes *p_ptr = &p;
    int c = 0;
    while (p_ptr != NULL)
    {
        if (p_ptr->layer == Fruit)
            c++;

        p_ptr = p_ptr->further_layers;
    }
...
bool is_stable(PileOfPancakes p)
{
    ...
    PileOfPancakes *p_ptr = &p;
    while (p_ptr->further_layers != NULL)
        p_ptr = p_ptr->further_layers;

    if (p_ptr->layer != ActualPancake)
        return false;
