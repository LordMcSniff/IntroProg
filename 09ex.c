...
{
    // das functioniert nicht, weil der stack nach der function out of scope geht
...
{
    PileOfPancakes *p_ptr = p.further_layers; // start with second layer, since the first is stack allocated
    while (p_ptr != NULL)
    {
        void *tmp = (void *)p_ptr;
...
PileOfPancakes create_pancake(uint32_t n)
{
    ...
    {
        PileOfPancakes *tmp_ptr = calloc(1, sizeof(PileOfPancakes));

        p_ptr->further_layers = tmp_ptr;
        p_ptr = tmp_ptr;
    }
...
{
    uint16_t *arr = calloc(x, sizeof(uint16_t));
